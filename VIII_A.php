<?php 
    ...
    function frist_number($value)
    {
        ...
        {
            $racine = racine_entiere_carre($value);
            $bool = true;
            $i = 2;
            while (($i <= $racine) AND $bool)
            {
                if (fmod($value, $i) == 0)
                {
                    $bool = false;
    ...
    if (isset($_POST['number']))
    {
        ...
        $first_number = frist_number($number);
...
<body>
    ...
    <div class="contact-clean">
        <form method="post">
            <h2 class="text-center">VIII A (Nombre premier)</h2>
            ...
            <?php
                if (isset($_POST['number']))
                {
                    echo "L'entier <strong>" . $number . "</strong> est-il un nombre premier ?";
                }
                else
                {
                   $result = "";
